...
from sklearn.metrics import r2_score, PredictionErrorDisplay, mean_absolute_error, mean_squared_error
...
file_path = 'D:/pythonProjects/solarPowerForecasting/mainProject/data_prepration/farm/sf_dataset.py'
...
def prepare_data(data, frame_size):
    X, y = [], []
    for i in range(frame_size, data.shape[0]):
        X.append(data.iloc[i - frame_size:i + 1, :-1].to_numpy())
        y.append(data.iloc[i, -1])
    return np.array(X), np.array(y)


X_train, y_train = prepare_data(train_scaled, look_back)
X_test, y_test = prepare_data(test_scaled, look_back)
...
def create_lstm_model():
    model = Sequential()
    model.add(LSTM(60, input_shape=(look_back + 1, num_features), return_sequences=True))
    ...
    model.add(Dense(1))
    opt = keras.optimizers.Adamax(learning_rate=3e-4, decay=1e-6)
...
history = model.fit(X_train, y_train, epochs=60, batch_size=16, verbose=2, validation_data=(X_test, y_test),
                    callbacks=[callback])
...
train_scaled = np.concatenate((X_train[:, -1, :], y_train.reshape(-1, 1)), axis=1)
